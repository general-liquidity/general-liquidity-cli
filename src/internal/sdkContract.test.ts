...
import { RAILS } from "./wire.ts";

/** Every runtime (non-type) export this CLI calls into. */
...
describe("installed @general-liquidity/sdk contract", () => {
  ...
  test("the rail vocabulary is non-empty and agrees with the SDK type", () => {
    // Direction checks are compile-time (see wire.ts); this only proves the literal survived.
...
type _RequiresClientSurface = Pick<
  GeneralLiquidity,
  "resolve" | "pay" | "verify" | "disclose" | "getAudit"
>;
...
type _RequiresProblemType = Problem["type"];
